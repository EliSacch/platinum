...
function StaffAppointmentEditForm({ setShow, query, setQuery, editId }) {
    ...
    useEffect(() => {
        const initializeForm = async () => {
            try {
                /**
                 * We try to initialize the form with the appointment data.
                 */
                const { data } = await axiosReq.get(`/appointments/${editId}/`);
                const { owner, client_name, date, time, treatment, notes } = data;
                setAppointmentData(
                    { owner, client_name, date, time, treatment, notes }
                )
                setHasLoaded(true);
            } catch (err) {
                console.log(err);
